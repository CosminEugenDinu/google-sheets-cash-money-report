...
function main() {
  ...
  const rawDataSheets = repGenSprSheet.getSheets().filter((sheet) => {
    const sheetName = sheet.getSheetName();
    // get all sheets except Interface and settingsSheet
    return (
      sheetName !== INTERFACE_SHEET_NAME && sheetName !== SETTINGS_SHEET_NAME
    );
  });
  ...
  const rawDataSheet = repGenSprSheet.getSheetByName(
    companyAlias + RAWDATA_SHEET_SUFFIX
  );
  ...
  if (procedure === "renderReport") {
    ...
    try {
      ...
      throw new Error(
        `Procedure ${procedure} failed with:\n${e.message}` +
  ...
  if (procedure === "importData") {
    ...
    try {
      ...
      const identifierPattern = settings
        .getField("procedure.variable.value")
        .getByIndex(
          settings
            .getField("procedure.variable.name")
      ...
      const [sheetToImportTo] = rawDataSheets.filter(
        (sheet) =>
          sheet.getName() === company.get("alias") + RAWDATA_SHEET_SUFFIX
      );
    ...
    if (messages)
      for (const [msg, count] of messages) {
        log(count, msg);
      }
  }
...
function mergeDateRecords(records_1, records_2) {
  ...
  records_1.map((map1) => {
    ...
    if (!alsoInRecords_2) merged.push(map1);
  });
  records_2.map((map2) => merged.push(map2));
  return merged;
}
...
function updateRawDataSheetNames(rawDataSheets, computedNames) {
  rawDataSheets.map((sheet, i) => sheet.setName(computedNames[i]));
}
...
function defaultTemplate() {
  const LABEL_STYLE = {
    ...
    borders: [null, null, null, null, false, false],
  };
  ...
  const TEMPLATE = {
    ...
    tax_id: {
      ...
      target_element: { cell: [3, 4], style: { horizontalAlignment: "left" } },
    },
    reg_num: {
      label_element: {
        cell: [4, 2],
    ...
    date: {
      label_element: { cell: [12, 1], value: "DATA", style: LABEL_STYLE },
    },
    ref: {
      label_element: { cell: [12, 2], value: "NR", style: LABEL_STYLE },
    },
    doc_type: {
      label_element: { cell: [12, 3], value: "TIP", style: LABEL_STYLE },
    },
    previous_balance: {
...
function libraryGet(required) {
  ...
  function addMessages(procedure) {
    ...
    const addMessage = (message) => {
      if (procedure.messages.has(message)) ++procedure.messages.get(message)[0];
      else procedure.messages.set(message, [1]);
  ...
  class Settings {
    constructor(settingsSheet, rowLim, colLim) {
      ...
      sheetValues[0].forEach((fieldName, index) => {
      ...
      for (const [fieldName, index] of fieldNames) {
        ...
        const fieldObject = {
          getByIndex(index) {
            if (!values.has(index))
              throw new ReferenceError(
                `Field ${fieldName} does not have key-index ${index}`
              );
          ...
          getByValue(val) {
            if (!indexes.has(val))
              throw new ReferenceError(
                `Field ${fieldName} does not have key-value ${val}`
              );
  ...
  function renderReport(
    ...
    class Element {
      ...
      constructor(typeKey, elem) {
        ...
        if (!this.supportedTypes.includes(typeKey))
          throw new TypeError(
            `${typeKey} is not a valid element type` +
        ...
        for (const prop of Element._typesProps.get(typeKey)) this[prop] = null;
        ...
        for (const p in elem) {
          if (typesProps.get(this._type).includes(p)) {
            ...
            throw new TypeError(
              `Property ${p} is not a supported by element type ${this._type}`
            );
          }
        }
      }
      ...
      static setProperty(range, property, value) {
        ...
        properties.set("horizontalAlignment", () =>
          range.setHorizontalAlignment(value)
        );
        properties.set("verticalAlignment", () =>
      ...
      render(sheet) {
        if (!this.cell)
          throw new TypeError(
            `Cannot render element when element.cell=${cell}`
          );
        ...
        if (this.type === "frame_element") {
          if (this.extent) range = sheet.getRange(...this.cell, ...this.extent);
        }
    ...
    Element._typesProps.set("frame_element", ["cell", "extent", "style"]);
    ...
    class DailyReport {
      ...
      constructor(date, company, dataRecords, calculateBalance) {
        ...
        this.previous_balance =
          Math.round((this.previous_balance + Number.EPSILON) * 100) / 100;
        ...
        if (this.day_balance < 0) {
          v > 0 &&
            addMessage(
              `WARNING ! Current day (${this.date.toLocaleDateString(
                "ro-RO"
              )}) balance is ${this.day_balance}`
            );
        }
      }
      ...
      static objToMap(obj, leafKeys, leaves = new Map()) {
        ...
        for (const key in obj) {
          ...
          // does not recurses on arrays; are passed over
            ...
            const [subTree, _leaves] = DailyReport.objToMap(
              obj[key],
              leafKeys,
              leaves
            );
      ...
      render(toSheet, template) {
        if (!this.dayValues)
          throw new TypeError(
            `Cannot render {DailyReport} instance if data values is ${this.dayValues}`
          );
        ...
        tree.get("companyName").get("target_element").value =
          company.get("name");
        tree.get("tax_id").get("target_element").value = company.get("tax_id");
        tree.get("reg_num").get("target_element").value =
          company.get("reg_num");
        ...
        ((group = tree.get("previous_balance")) => {
          ...
          else label.value = label.value.replace(/luna\//i, "");
        ...
        for (const record of this.dayValues) {
          for (const [parentKey, elementType] of tree.get("record")) {
            ...
            if (parentKey === "input")
              newRecElem.value =
                record.get("I_O_type") === 1 ? record.get("value") : "";
            if (parentKey === "output")
              newRecElem.value =
                record.get("I_O_type") === 0 ? record.get("value") : "";
        ...
        ((group = tree.get("day_total_label")) => {
          ...
          label.value = replaceCurly(
            label.value,
            this.date.toLocaleDateString("ro-RO")
          );
        ...
        ((group = tree.get("day_total_targets")) => {
          ...
          const total_day_output = group
            .get("total_day_output")
            .get("target_element");
      ...
      balanceCalculator() {
        ...
        return (currentDateStr) => {
          ...
          for (const dateStr of sortedDates) {
            if (currentDateStr < dateStr) return total;
              ...
              if (typeof recordValue !== "number")
                throw new TypeError(
                  `Expected Number! Typeof record value ${recordValue}: ${typeof recordValue}. ` +
                    `Date key: ${dateStr}. Local Date: ${new Date(
                      dateStr
                    ).toLocaleDateString("ro-RO")}`
                );

              if (record.get("I_O_type") === 1) return dayTotal + recordValue;
              if (record.get("I_O_type") === 0) return dayTotal - recordValue;
            }, 0);
          }
          /******************
      ...
      render(targetSpreadsheet, template) {
        ...
        targetSpreadsheet.getSheets().forEach((sheet) => {
          if (sheet.getIndex() === 1)
            // cover sheet
            sheet.setName("Cover");
          else targetSpreadsheet.deleteSheet(sheet);
        });
        const dates = datesBetween(fromDate, toDate);

        v > 1 &&
          addMessage(
            `Rendering reports between ${fromDate.toLocaleDateString(
              "ro-RO"
            )} and ${toDate.toLocaleDateString("ro-RO")}`
          );
        ...
        for (const date of dates) {
          ...
          v > 1 &&
            addMessage(
              `Day report ${new Date(date).toLocaleDateString(
                "ro-RO"
              )} rendered!`
            );
        }
  ...
  function importData(
    ...
    const srcSpreadsheets = sheetIds.reduce((spreadsheets, sheetId) => {
      try {
    ...
    if (!srcSpreadsheets.length) {
      ...
      v > 0 &&
        addMessage(
          `Spreadsheets opened ${
            srcSpreadsheets.length
          }, [${srcSpreadsheets.map((ss) => ss.getName())}]`
        );
    }
    ...
    for (const sheet of srcSpreadsheets) {
      ...
      if (searchRecords.messages) {
        for (const [mes, count] of searchRecords.messages) addMessage(mes);
      }
    }
    ...
    for (const [dateStr, records] of foundRecords) {
      for (const recordMap of records) {
        ...
        for (const fieldName in targetFieldNames) {
          ...
          if (fieldName === "date") newRecord[fieldIndex] = new Date(dateStr);
          else newRecord[fieldIndex] = recordMap.get(fieldName);
        }
    ...
    while (isEmpty) {
      ...
      for (const fieldName in targetFieldNames) {
        const fieldIndex = targetFieldNames[fieldName];
        if (
          ![NaN, null, undefined, ""].includes(targetValues[last][fieldIndex])
        )
          isEmpty = false;
      }
    }
    ...
    const newTargetRange = sheetToImportTo.getRange(
      fromRow,
      fromCol,
      numOfRows,
      numOfCols
    ...
    for (const sheet of spreadsheet.getSheets()) {
      ...
      if (sheetRecords.length) {
        ...
        sheetRecords.map((record) => {
          ...
          if (input) recordMap.set("I_O_type", 1);
          else if (output) recordMap.set("I_O_type", 0);
  ...
  function cleanRawData(fromDate, toDate, company, rawDataSheet) {
    ...
    v > 1 &&
      addMessage(`values retrieved from sheet ${rawDataSheet.getName()}`);
  ...
  *************/
    ...
    for (const fieldName in fieldNames) {
      ...
      else throw new Error(`Unknown fieldName ${fieldName}`);
    }

    const convertType = (recordVal, newType) => {
      const currentType = getType(recordVal);
      if (currentType === newType) return recordVal;
    ...
    while (++row_i < values.length) {
      ...
      for (const fieldName in fieldNames) {
        ...
        try {
          ...
          if (getType(e) === "TypeError") {
            ...
            v > 1 &&
              addMessage(
                `converted {${getType(testValue)}} ${testValue} ` +
      ...
      if (!uniques.has(recordHash)) uniques.set(recordHash, row_i);
    }

    v > 1 &&
      addMessage(
        `Deleted all values from fieldIndexes except:[${fieldIndexes.filter(
          (i) => i !== undefined
        )}]`
      );

    v > 1 &&
      addMessage(`nonEmptyRows ${nonEmptyRows} <- without fields names row`);
    v > 1 &&
      addMessage(
        `uniques.size ${uniques.size} <- this includes first row - fields names`
      );
    ...
    v > 1 &&
      addMessage(`Duplicates removed ${nonEmptyRows + 1 - uniques.size}`);
    v > 1 &&
      addMessage(
        `newValues.length ${newValues.length} <- this includes first row - fields names`
      );
    ...
    v > 0 &&
      addMessage(`procedure done in ${(Date.now() - startTimer) / 1000} sec`);
  ...
  function getType(obj) {
    ...
    if (Object.is(obj, Boolean(obj))) return "boolean";
  ...
  class FieldValidator {
    ...
    setField(
      ...
      if (isNone(minValueType) && isNone(maxValueType)) {
        ...
        // here we got not none type for last arg (exactValues)
  ...
  function validateRecord(record, fieldNames) {
    ...
    validators.set("date", (date) => {
      if (isNaN(date.getTime()))
        throw new TypeError(
          `In field ${
            fieldNames["date"]
          } ${typeof date} ${date} is not valid Date.`
        );
    });

    validators.set("ref", (ref, fieldName) => {
      ...
      if (type !== expectedType) errorInfo.errors.push("type_error");
      //throw new TypeError(JSON.stringify({...errorInfo, ));

      if (errorInfo.errors.length) throw new Error(JSON.stringify(errorInfo));
    });

    validators.set("doc_type", (doc_type) => {
    ...
    validators.set("I_O_type", (I_O_type) => {
    ...
    for (const fieldName in fieldNames) {
      ...
      if (!validators.has(fieldName))
        throw new ReferenceError(
          `Validator function not set for field ${fieldName}`
        );
  ...
  function getFieldNames(firstRow) {
    ...
    const fieldNames = firstRow.reduce((indexes, fieldName, i) => {
  ...
  function getRecords(rawDataSheet, fieldDescriptors) {
    ...
    v > 0 &&
      addMessage(`Records in ${rawDataSheet.getName()}: ${values.length}`);
    ...
    for (let row_i = 1; row_i < values.length; row_i++) {
      ...
      for (const fieldName of expectedFieldNames) {
        ...
        try {
          validator.validate(fieldName, thisValue);
        } catch (err) {
